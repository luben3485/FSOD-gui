...
class SimulationLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # negative supports
        unseen_class = [2, 5, 10, 14]
        if self.support_way != 1:
            neg_cls = []
            for cls_id in range(1, self._num_classes):
                if cls_id not in cls_in_query and cls_id not in unseen_class:
        ...
        # filt boxes
        all_cls_gt_boxes = gt_boxes.copy()
        pos_box_idx = []
        for i in range(gt_boxes.shape[0]):
            if gt_boxes[i, 4] == pos_cls_idx:
                pos_box_idx.append(i)
        gt_boxes_new = np.zeros((len(pos_box_idx), 5))
        for i, box_idx in enumerate(pos_box_idx):
            gt_boxes_new[i] = gt_boxes[box_idx]
            gt_boxes_new[i, 4] = 1.
        gt_boxes = torch.from_numpy(gt_boxes_new)
        all_cls_gt_boxes = torch.from_numpy(all_cls_gt_boxes)
        ...
        if self._roidb[index_ratio]['need_crop']:
            if ratio < 1:
                ...
                all_cls_gt_boxes[:, 1] = all_cls_gt_boxes[:, 1] - float(y_s)
                all_cls_gt_boxes[:, 3] = all_cls_gt_boxes[:, 3] - float(y_s)

                # update gt bounding box according the trip
                gt_boxes[:, 1].clamp_(0, trim_size - 1)
                gt_boxes[:, 3].clamp_(0, trim_size - 1)
                all_cls_gt_boxes[:, 1].clamp_(0, trim_size - 1)
                all_cls_gt_boxes[:, 3].clamp_(0, trim_size - 1)
                ...
                all_cls_gt_boxes[:, 0] = all_cls_gt_boxes[:, 0] - float(x_s)
                all_cls_gt_boxes[:, 2] = all_cls_gt_boxes[:, 2] - float(x_s)
                # update gt bounding box according the trip
                gt_boxes[:, 0].clamp_(0, trim_size - 1)
                gt_boxes[:, 2].clamp_(0, trim_size - 1)
                all_cls_gt_boxes[:, 0].clamp_(0, trim_size - 1)
                all_cls_gt_boxes[:, 2].clamp_(0, trim_size - 1)
        ...
        if ratio < 1:
            ...
            all_cls_gt_boxes[:, :4].clamp_(0, trim_size)
        ...
        #
        not_keep = (all_cls_gt_boxes[:,0] == all_cls_gt_boxes[:,2]) | (all_cls_gt_boxes[:,1] == all_cls_gt_boxes[:,3])
        keep = torch.nonzero(not_keep == 0).view(-1)

        all_cls_gt_boxes_padding = torch.FloatTensor(self.max_num_box, all_cls_gt_boxes.size(1)).zero_()
        if keep.numel() != 0:
            all_cls_gt_boxes = all_cls_gt_boxes[keep]
            num_boxes = min(all_cls_gt_boxes.size(0), self.max_num_box)
            all_cls_gt_boxes_padding[:num_boxes,:] = all_cls_gt_boxes[:num_boxes]
        else:
            num_boxes = 0
        ...
        return padding_data, im_info, gt_boxes_padding, num_boxes, support_data_all, all_cls_gt_boxes_padding
