...
class _reweight_RCNN(nn.Module):
    """ faster RCNN """
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, support_ims, all_cls_gt_boxes=None):
        ...
        if self.training:
            ...
            pos_support_feat = self.shot_reweight(pos_feat_list)  # [B, 1024, 20, 20]
            neg_support_feat = self.shot_reweight(neg_feat_list)
    ...
    def shot_reweight(self, feat_list):
        cosine_similarity = nn.CosineSimilarity(dim=1, eps=1e-6)
        batch_size = feat_list[0].size(0)
        n_shot = len(feat_list)

        prorotype = torch.stack(feat_list, 0).mean(0).view(batch_size, -1)  # [B, 1024, 20, 20]
        similarity_each_shot = []
        total = 0.
        for i in range(n_shot):
            curr_shot_feat = feat_list[i].view(batch_size, -1)
            cos_sim = cosine_similarity(curr_shot_feat, prorotype)  # [B]
            cos_sim = torch.exp(cos_sim * 10)
            similarity_each_shot.append(cos_sim)
            total += cos_sim

        final_feat = 0.
        for i in range(n_shot):
            weight = similarity_each_shot[i] / total
            final_feat += feat_list[i] * weight.view(batch_size, 1, 1, 1)
        ...
        #         feat_2 = feat_list[j]
        #         relation_score = torch.bmm(feat_1.view(batch_size, 1, -1), feat_2.view(batch_size, -1, 1))
        #         gram_list.append(([i, j], relation_score.squeeze()))
    
        # score_each_shot = []
        # total_score = 0
        # for i in range(n_shot):
        #     curr_shot_score = 0.
        #     for pair, sc in gram_list:
        #         if i in pair:
        #             curr_shot_score += sc
        #     score_each_shot.append(curr_shot_score)
        #     total_score += curr_shot_score

        # weight_each_shot = [_sc / total_score for _sc in score_each_shot]

        # weighted_shot_feat = 0
        # for i in range(n_shot):
        #     weighted_shot_feat += feat_list[i] * weight_each_shot[i].view(batch_size, 1, 1, 1)  # [B, c, 20, 20]

        # return weighted_shot_feat


        
class ReweightFSOD(_reweight_RCNN):
  def __init__(self, classes, num_layers=50, pretrained=False, num_way=2, num_shot=5, g=True, l=True, p=True):
    ...
    _reweight_RCNN.__init__(self, classes, n_way=num_way, n_shot=num_shot, g=g, l=l, p=p)
