...
class _share1RCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, support_ims, all_cls_gt_boxes=None):
        ...
        q_mat_s = self.Q_weight(support_mat)  # [B, 400, d_k]
        k_mat_s = self.K_weight(support_mat)
        v_mat_s = self.V_weight(support_mat)
        qk_mat_s = torch.bmm(q_mat_s, k_mat_s.transpose(1, 2)) / math.sqrt(self.d_k)  # [B, 400, 400]
        qk_mat_s = F.softmax(qk_mat_s, dim=2)
        z_mat_s = torch.bmm(qk_mat_s, v_mat_s)  # [B, 400, d_k]

        q_mat_q = self.Q_weight(query_mat)
        k_mat_q = self.K_weight(query_mat)
        v_mat_q = self.V_weight(query_mat)
        qk_mat_q = torch.bmm(q_mat_q, k_mat_q.transpose(1, 2)) / math.sqrt(self.d_k)  # [B, h*w, h*w]
        qk_mat_q = F.softmax(qk_mat_q, dim=2)
        z_mat_q = torch.bmm(qk_mat_q, v_mat_q)  # [B, h*w, d_k]

        double_correlation_feat = torch.bmm(z_mat_q, z_mat_s.transpose(1, 2)) / math.sqrt(self.d_k)  # [B, h*w, 400]
        double_correlation_feat = double_correlation_feat.transpose(1, 2).view(batch_size, -1, feat_h, feat_w)  # [B, 400, h, w]
        rois, rpn_loss_cls, rpn_loss_bbox = self.RCNN_rpn(double_correlation_feat, im_info, gt_boxes, num_boxes)
    ...
    def rcnn_head(self, pooled_feat, support_feat):
        ...
        _q = self.Q_weight(support_mat)
        _k = self.K_weight(query_mat)
        _v = self.V_weight(query_mat)
        _qk = torch.bmm(_q, _k.transpose(1, 2)) / math.sqrt(self.d_k)
        _qk = F.softmax(_qk, dim=2)
        _z = torch.bmm(_qk, _v).view(n_roi, -1)  # [B*128, 49*d_k]
        
        cls_score = self.ffn_layer(_z)
...
class Share1RCNN(_share1RCNN):
    def __init__(self, classes, num_layers=50, pretrained=False, num_way=2, num_shot=5, pos_encoding=True):
        self.model_path = 'data/pretrained_model/resnet50_caffe.pth'
        self.dout_base_model = 1024
        self.pretrained = pretrained
        _share1RCNN.__init__(self, classes, n_way=num_way, n_shot=num_shot, pos_encoding=pos_encoding)
